...
```{r}
# create a logical mask for each corpus of size 1%
...
sample_twitter <- corpus_sample(corpus_t, p=0.01)
sample_blog <- corpus_sample(corpus_b, p=0.01)
sample_news <- corpus_sample(corpus_n, p=0.01)

# aggregate samples into a single corpus
scfile <- file.path("~", "Data Science Coursera", "capstone-JHU-swiftkey", "JHU-DSS-capstone", "data", "sample_corpus.txt")
...
ngram <- function(n) {
    ...
    dtm_ngram <- DocumentTermMatrix(scorp, 
     ...
     <- as.data.frame(apply(my_function_tdm,1,sum))
    colnames(my_function_tdm) <- c("Qty")
    return(my_function_tdm)
    return(dtm_ngram)
}
